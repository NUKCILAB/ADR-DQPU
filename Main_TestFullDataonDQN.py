...
class CustomTabularEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.DataNumNow +=1
        self.current_state = self.x[self.DataNumNow]
        self.TP = 0
        self.TN = 0
        self.FP = 0
        self.FN = 0
    ...
    def step(self, action):
        ...
        self.DataNumNow +=1
        next_state = self.x[self.DataNumNow] # generate next state
        if self.counts>=self.m_feature: # For Test ,All data -1
          done = True
        else:
          done = False # set done flag to False
        if self.counts%1000 == 0:
            print(self.counts,"/",self.m_feature)
    ...
    def calculate_reward1(self, state, action):
        Adr = self.ADRs(state)
        #if Adr >=4:
            #self.y[self.DataNumNow] = 1
        if action == 1 :
            if self.y[self.DataNumNow] == 1:
                self.TP += 1
                self.PredictLabel=np.append(self.PredictLabel,1)
                self.TrueLabel=np.append(self.TrueLabel,1)
                return 1
            elif Adr ==(6/6):
                self.y[self.DataNumNow] = 1
                self.TP += 1
                ...
                self.FP += 1
                self.PredictLabel=np.append(self.PredictLabel,1)
                self.TrueLabel=np.append(self.TrueLabel,0)
                return -1+Adr
        elif action == 0 :
            if self.y[self.DataNumNow] == 0:
                self.TN += 1
                self.PredictLabel=np.append(self.PredictLabel,0)
                self.TrueLabel=np.append(self.TrueLabel,0)
                return 1
            elif Adr <=(1/6):
                self.y[self.DataNumNow] = 0
                self.TN += 1
                self.PredictLabel=np.append(self.PredictLabel,0)
                self.TrueLabel=np.append(self.TrueLabel,0)
                return 0.8
    ...
    def PrintTrueLabel(self):
        return self.TrueLabel
    
    def PrintPredictLabel(self):
...
model.add(Dense(12,kernel_regularizer=regularizers.l1(0.01)))
...
dqn.compile(optimizer=Adam(learning_rate=3e-4))#,jit_compile=True)#,metrics=['accuracy'])
...
if Test :
    ...
    #testdata = pd.read_csv('.\data\ATC_N05AX12_C0015732.csv', header=None)
        ...
        for k in  range(Test_episode):
            ...
            if Test_inf :
                ...
                if k == Test_episode-1:
                    ...
                    y = env.PrintTrueLabel()
                    y_pred = env.PrintPredictLabel()
                    #print(y)
                    #print(y_pred)
                    print("ROC AUC Score : ",round(roc_auc_score(y, y_pred),4))
                    print("Average Precision Score : ",round(average_precision_score(y, y_pred),4))
                    
                else:
                    print("Episodes:",k+1,"/",Test_episode)    
    except OSError:
